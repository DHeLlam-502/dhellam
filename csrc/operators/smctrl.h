...
constexpr uint64_t calculateMask(unsigned int count, bool reverse) {
    if (count == 0) return 0;
    return reverse?( (1ULL << count/2) - 1 ):~( (1ULL << count/2) - 1 );
}

#define SET_SM_COUNT(count,reverse) libsmctrl_set_next_mask(calculateMask(count,reverse))
