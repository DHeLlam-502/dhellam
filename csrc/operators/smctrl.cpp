...
static void setup_sm_control_11() {
	...
	subscribe = reinterpret_cast<decltype(subscribe)>(subscribe_func_addr);
	enable = reinterpret_cast<decltype(enable)>(enable_func_addr);
	int res = 0;
...
// void pyset_next_mask(unsigned int count, bool reverse){
//     SET_SM_COUNT(count, reverse);
// }